...
export class AffinityService {
  ...
  /**
   * Fetch and cache all field data for an opportunity in one API call
   * Uses the v2 API embedded fields to minimize API calls
   */
  async fetchAndCacheOpportunityFieldData(opportunityId: number): Promise<any> {
    try {
      // First check if we have cached data
      const { storage } = await import('../storage');
      const cachedData = await storage.getAffinityFieldData(opportunityId.toString());
      
      if (cachedData) {
        console.log(`[CACHE HIT] Using cached field data for opportunity ${opportunityId}`);
        return cachedData;
      }

      console.log(`[CACHE MISS] Fetching field data for opportunity ${opportunityId}`);
      
      // Fetch the opportunity with embedded fields from v2 API
      ...
      if (opportunity.organization_id) {
        try {
          const orgFieldValues = await this.getFieldValues(opportunity.organization_id, 'organization');
          organizationData = orgFieldValues;
        } catch (error) {
          console.log(`[WARNING] Could not fetch organization data for ${opportunity.organization_id}: ${error.message}`);
        }
      }
      
      // Cache the complete field data
      const cacheData = {
        affinityId: opportunityId.toString(),
        recordType: 'opportunity',
        fieldData: fieldData,
        organizationData: organizationData,
        personData: null, // Not applicable for opportunities
        affinityLastModified: new Date(opportunity.last_modified || new Date())
      };
      
      const cached = await storage.createOrUpdateAffinityFieldData(cacheData);
      console.log(`[CACHE STORED] Cached field data for opportunity ${opportunityId}`);
      
      return cached;
    } catch (error) {
      console.error(`[ERROR] Failed to fetch field data for opportunity ${opportunityId}:`, error);
      throw error;
    }
  }

  /**
   * Get specific field values from cached data
   * Only syncs the fields that are actually mapped in the sync configuration
   */
  async getFieldValuesFromCache(opportunityId: number, fieldMappings: any[]): Promise<any[]> {
    const cachedData = await this.fetchAndCacheOpportunityFieldData(opportunityId);
    
    const fieldValues = [];
    
    for (const mapping of fieldMappings) {
      const fieldId = mapping.affinityFieldId;
      
      if (typeof fieldId === 'number' && fieldId > 0) {
        // Organization field - extract from cached organization data
        if (cachedData.organizationData) {
          const orgField = cachedData.organizationData.find(f => f.field_id === fieldId);
          if (orgField) {
            fieldValues.push({
              field_id: fieldId,
              value: orgField.value,
              field_name: mapping.affinityField
            });
          }
        }
      } else if (typeof fieldId === 'string' && fieldId.startsWith('field-')) {
        // Opportunity field - extract from cached field data
        const oppField = cachedData.fieldData.find(f => f.id === fieldId);
        if (oppField) {
          fieldValues.push({
            field_id: fieldId,
      ...
      } else if (fieldId === 'companies') {
        // Special handling for companies field
        fieldValues.push({
          field_id: 'companies',
          value: cachedData.fieldData.find(f => f.id === 'companies')?.value || [],
          field_name: mapping.affinityField
        });
      }
    }
